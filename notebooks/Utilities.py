...
def ToDisk(varObject,strForcedName="",strType=""):
    ...
    if strForcedName == "":
      ...
      if strType == "":
        ...
        print("Strange type in function ToDisk().")
...
def FromDisk(strObjectName):
    ...
    2022 01 31: DH Created
    '''
    print("Function u.FromDisk() must be adjusted to Cookiecutter first.")    
    strFilename = f"{gcstrPicklePath}{strObjectName}.p"
...
def ShortDataFrameDisplay(dfrSource,cintExamples=5):
    ...
    '''
    print(f"DataFrame '{VariableName(dfrSource)}':")
    pd.options.display.max_rows = cintExamples
    DisplayDataFrame(dfrSource)
    pd.reset_option('display.max_rows')    
    
import matplotlib.pyplot as plt
import pandas as pd
def DrawPredictions2023Q1(dfrSource,dstrColors):
    '''
    ...
    '''
    # get test data
    objStartDate = pd.to_datetime('2023-01-01')
    objEndDate = pd.to_datetime('2023-03-31')
    dfrTest = dfrSource[(dfrSource['Date'] >= objStartDate) & (dfrSource['Date'] <= objEndDate)]

    # create plot
    plt.figure(figsize=(15, 6))
    for strColumn,strColor in dstrColors.items():
        plt.plot(dfrTest['Date'], dfrTest[strColumn], label=strColumn, color=strColor)

    # customize plot
    strPredictionWord = "prediction" if len(dstrColors) == 2 else "predictions"
    plt.xlabel('Date')
    plt.ylabel('Price')
    plt.title(f'True price vs. {strPredictionWord}\n2023 Q1')
    plt.legend()
